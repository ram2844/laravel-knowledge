	                                ...
	                                <table class="table">
	                                    <thead>
	                                        <tr>
	                                            ...
	                                            <th class="pl-0 font-weight-bold text-muted  text-uppercase">Product Size</th>
	                                    	...
	                                    	@if($orderDetails->count())
		                                    	@foreach($orderDetails as $key => $value)
			                                        <tr class="font-weight-boldest">
			                                            <td class="pl-0 pt-7 d-flex align-items-center">
			                                                ...
			                                                <div class="symbol symbol-40 flex-shrink-0 mr-4 bg-light">
				                                                @if(isset($value->product_image) && !empty($value->product_image))
																	<div class="symbol-label" style="background-image: url({{asset('uploads/ecomm_products/thumbnails/250/'.$value->product_image)}})"></div>
							                            		@else
			                                            ...
			                                            <td class="text-left pt-7 align-middle"> {{$value->product_name}}</td>
			                                            <td class="text-left pt-7 align-middle"> {{$value->product_size }}</td>
			                                            <td class="pr-0 pt-7 text-left align-middle"><i class='fas fa-rupee-sign icon-sm'></i> {{ number_format($value->product_price, 2) }}</td>

			                                            <td class="pr-0 pt-7 text-left align-middle">{{ $value->product_qty ?? 1 }}</td>

			                                            <td class="pr-0 pt-7 text-left align-middle"><i class='fas fa-rupee-sign icon-sm'></i> {{ number_format(($value->product_total_amount), 2) }}</td>
			                                        </tr>
		                                        @endforeach
	                                        @endif
		                        	...
		                        	<div class="form-group row validated">
			                            ...
			                            <div class="col-lg-6 col-md-6 col-sm-12">
			                                <select class="form-control selectpicker" name="status" >
			                                    ...
			                                    <option {{ $row->status == 'CONFIRM' ? 'selected' : '' }} value="CONFIRMED">Confirmed</option>
			                                    <option {{ $row->status == 'COMPLETED' ? 'selected' : '' }} value="COMPLETED">Completed</option>
			                                    <option {{ $row->status == 'FAILED' ? 'selected' : '' }} value="FAILED">Rejected</option>
			                                    <option {{ $row->status == 'CANCELLED' ? 'selected' : '' }} value="CANCELLED">Rejected</option>
