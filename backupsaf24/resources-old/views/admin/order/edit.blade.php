	                                    	...
	                                    	@if($orderDetails->count())
		                                    	@foreach($orderDetails as $key => $value)
			                                        <tr class="font-weight-boldest">
			                                            <td class="pl-0 pt-7 d-flex">
			                                            	<a href="{{ route('program.details', ['id' => $value->program->slug]) }}" target="_blank" style="color: #3f4254;">
				                                                <!--begin::Symbol-->
			                                            ...
			                                            <td class="text-left pt-7"> {{$value->getFormattedDateTime() }}</td>
			                                            <td class="pr-0 pt-7"><i class='fas fa-rupee-sign icon-sm'></i> {{ number_format($value->program_amount, 2) }}</td>

			                                            <td class="pr-0 pt-7">{{ $value->program_qty ?? 1 }}</td>

			                                            <td class="pr-0 pt-7"><i class='fas fa-rupee-sign icon-sm'></i> {{ number_format(($value->program_total_amount), 2) }}</td>
			                                        </tr>
		                                        @endforeach
	                                        @endif
		                        ...
		                        <div class="col-md-11">
		                            <div class="table-responsive">
		                                <table class="table">
		                                    <thead>
		                                        <tr>
		                                            <th class="pl-0 font-weight-bold text-muted  text-uppercase">COUPON CODE</th>
		                                            <th class="pl-0 font-weight-bold text-muted  text-uppercase text-right">COUPON AMOUNT</th>
		                                    ...
		                                    <tbody>
		                                        <tr class="font-weight-bolder">
		                                            ...
		                                            <td class=" text-right"><i class='fas fa-rupee-sign'></i> {{ number_format($row->coupon_amount, 2) }}</td>
	                                ...
	                                <table class="table">
	                                    <thead>
	                                        <tr>
	                                            ...
	                                            <th class="pl-0 font-weight-bold text-muted  text-uppercase text-right">TOTAL AMOUNT</th>
	                                    ...
	                                    <tbody>
	                                        <tr class="font-weight-bolder">
	                                            ...
	                                            <td class="text-right"><i class='fas fa-rupee-sign'></i> {{ number_format(($row->total_amount + $row->coupon_amount), 2) }}</td>
		                        	...
		                        	<div class="form-group row validated">
			                            ...
			                            <div class="col-lg-6 col-md-6 col-sm-12">
			                                <select class="form-control selectpicker" name="status" >
			                                    ...
			                                    <option {{ $row->status == 'SUCCESS' ? 'selected' : '' }} value="SUCCESS">Success</option>
			                                    <option {{ $row->status == 'FAILED' ? 'selected' : '' }} value="FAILED">Failed</option>
