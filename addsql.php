...
<div class="d-flex flex-column-fluid">
    ...
    <div class="form-group row validated">
        ...
        <div class="col-lg-9 col-md-9 col-sm-12">
            <input type="text" name="pincode" value="{{ old('pincode', $row->pincode ?? '') }}" class="form-control form-control-lg form-control-solid @error('pincode') is-invalid @enderror " placeholder="Enter Pincode" required oninput="getPincodeData()"/>
            @error('pincode')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
...
<div class="col-12">
    <div class="form-group row validated">
        ...
        <div class="col-lg-9 col-md-9 col-sm-12">
            <select class="form-control selectpicker" name="country_id" data-country-id="{{ old('country_id', $row->country_id ?? '') }}" tabindex="null" onchange="getState()">
...
<script type="text/javascript">
    ...
    if (pincode) {
        $.ajax({
            type: 'GET',
            url: "{{ route('ajax.fetch.pincode') }}",
            data: { pincode: pincode },
            success: function (response) {
                if (response.status) {
                    var data = response?.data[0];
                    $("select[name='country_id']").val(data?.country_id).selectpicker('refresh');
                    $("select[name='state_id']").val(data?.state_id).selectpicker('refresh');
                    getState(); // Update state dropdown
                    $("select[name='city_id']").val(data?.city_id).selectpicker('refresh');
                }
            },
